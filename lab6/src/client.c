...
int recognizing(char* buff, uint32_t* pos, struct Server* to)
{
	char* temp = buff;
	uint32_t offset = 0;
	for(int j = 0; j < 3; j++)
...
int main(int argc, char **argv) {
  ...
  // delete this and parallel work between servers
	...
	while(!feof(f) || (servers_num > k))
	{
		...
		if(recognizing(buff, &pos, to + servers_num))
		{
			fclose(f);
  ...
  // work continiously, rewrite to make parallel
